[EXEC_SUMMARY_cdc.md](https://github.com/user-attachments/files/22052424/EXEC_SUMMARY_cdc.md)
...
- **Module:** g-oss-cdc (Change Data Capture & replication)
- **Purpose:** Real-time database changes into open Kafka topics and Iceberg tables.
- **Primary components:** Debezium, Kafka/Redpanda, Kafka Connect, Iceberg Sink, Apicurio Registry, Postgres (sample)
- **Replaces / reduces:** Oracle GoldenGate, Qlik Replicate, Informatica CDC pipelines
- **Integrates with:** g-oss-core, g-oss-batch, g-oss-governance, g-oss-observability, g-oss-security
...
```
[OLTP DB] --CDC→ Debezium → Kafka → Iceberg Sink → Iceberg tables (g-oss-core)
                                      ↑ Apicurio schema registry
```

This repository delivers the **g-oss-cdc (Change Data Capture & replication)** segment of that picture.

---

## 4) Common oil & gas use cases (examples)

- Well tests and volumes mirrored near-real-time
- Work orders / field tickets updates for ops dashboards
- Subledger deltas for intraday finance checks
...
- End-to-end **CDC latency P95 ≤ 60s** (tighter for small tables).
- Registry enforces **backward-compatible schemas**; breaking changes blocked.
...
1. **Pilot (0–90 days)**  
   - Stand up `g-oss-cdc (Change Data Capture & replication)` in a sandbox; connect a *low-risk* data domain.  
...
- **Oracle privileges/log access** → Engage DBA early; test in lower envs.
- **Backfill complexity** → Use initial snapshot then CDC; validate counts.
- **Topic retention misconfig** → Monitor lag and retention; compact keys.
...
- GoldenGate/Qlik Replicate/Informatica CDC → Debezium + Kafka + Iceberg Sink
- Proprietary schema registries → Apicurio Registry
